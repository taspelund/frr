...
static void
zvni_print_neigh (zebra_neigh_t *n, void *ctxt, json_object *json);
static void
zvni_print_neigh_hash (struct hash_backet *backet, void *ctxt);
static void
zvni_print_neigh_hash_all_vni (struct hash_backet *backet, void **args);
static void
...
zvni_neigh_add (zebra_vni_t *zvni, struct ipaddr *ip, struct ethaddr *mac);
static int
...
zvni_neigh_send_add_to_client (struct zebra_vrf *zvrf, vni_t vni,
                               struct ipaddr *ip, struct ethaddr *macaddr, u_char flags);
static int
...
static int
zvni_del_macip_for_intf (struct interface *ifp, zebra_vni_t *zvni);
static int
zvni_add_macip_for_intf (struct interface *ifp, zebra_vni_t *zvni);
static int
zvni_gw_macip_add (struct interface *ifp, zebra_vni_t *zvni,
                   struct ethaddr *macaddr, struct ipaddr *ip);
static int
zvni_gw_macip_del (struct interface *ifp, zebra_vni_t *zvni, struct ipaddr *ip);
struct interface*
...
static int
advertise_gw_macip_enabled (struct zebra_vrf *zvrf, zebra_vni_t *zvni)
{
      ...
      if (json == NULL)
        {
          ...
          vty_out(vty, " State: %s", IS_ZEBRA_NEIGH_ACTIVE (n) ? "Active" : "Inactive");
        }
      else
        json_object_string_add (json, "remoteVtep", inet_ntoa (n->r_vtep_ip));
    }
  if (json == NULL)
...
zvni_print_mac (zebra_mac_t *mac, void *ctxt)
{
  ...
  if (!listcount (mac->neigh_list))
    vty_out(vty, "    No Neighbors %s", VTY_NEWLINE);
  else
    {
      for (ALL_LIST_ELEMENTS_RO (mac->neigh_list, node, n))
        {
          vty_out(vty, "    %s %s%s",
                  ipaddr2str (&n->ip, buf2, sizeof(buf2)),
                  CHECK_FLAG (n->flags, ZEBRA_MAC_LOCAL) ? (IS_ZEBRA_NEIGH_ACTIVE(n) ? "Active" : "Inactive") : "",
                  VTY_NEWLINE);
        }
    }
...
zvni_print (zebra_vni_t *zvni, void **ctxt)
{
  ...
  if (json == NULL)
    ...
    {
      json_object_string_add (json, "vxlanInterface", zvni->vxlan_if->name);
      json_object_int_add (json, "ifindex", zvni->vxlan_if->ifindex);
      json_object_string_add (json, "vtepIp", inet_ntoa(zvni->local_vtep_ip));
      json_object_string_add (json, "advertiseGatewayMacip", zvni->advertise_gw_macip ? "Yes" : "No");
      ...
      for (zvtep = zvni->vteps; zvtep; zvtep = zvtep->next)
        {
          if (json == NULL)
            ...
            {
              json_ip_str = json_object_new_string (inet_ntoa (zvtep->vtep_ip));
              json_object_array_add (json_vtep_list, json_ip_str);
            }
        }
      if (json)
        json_object_object_add (json, "numRemoteVteps", json_vtep_list);
    }
  if (json == NULL)
    {
      ...
      vty_out(vty, " Advertise-gw-macip: %s%s ",
              zvni->advertise_gw_macip ? "Yes" : "No", VTY_NEWLINE);
    }
}
      ...
      if (num_vteps)
        {
          ...
          json_object_object_add (json_vni, "remoteVteps", json_vtep_list);
        }
      json_object_object_add (json, vni_str, json_vni);
    }
}
                               ...
                               u_int16_t cmd)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("%u:Send MACIP %s flags 0x%x MAC %s IP %s VNI %u to %s",
...
static void
zvni_process_neigh_on_local_mac_add (struct zebra_vrf *zvrf,
                                     zebra_vni_t *zvni, zebra_mac_t *zmac)
{
  zebra_neigh_t               *n = NULL;
  struct listnode             *node = NULL;
  char                        buf[ETHER_ADDR_STRLEN];
  ...
  for (ALL_LIST_ELEMENTS_RO (zmac->neigh_list, node, n))
    {
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          /* MAC is learnt locally, program all inactive neigh pointing to this mac */
          if (IS_ZEBRA_NEIGH_INACTIVE (n))
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug ("%u: neigh %s (MAC %s) on VNI %u is now ACTIVE",
                            zvrf_id (zvrf),
                            ipaddr2str (&n->ip, buf2, sizeof(buf2)),
              ...
              zvni_neigh_send_add_to_client (zvrf, zvni->vni, &n->ip, &n->emac, 0);
            }
          else
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug ("%u: neigh %s (MAC %s) on VNI %u should NOT be ACTIVE",
                            zvrf_id (zvrf),
                            ipaddr2str (&n->ip, buf2, sizeof(buf2)),
                            prefix_mac2str (&n->emac, buf, sizeof (buf)),
                            zvni->vni);
...
static void
zvni_process_neigh_on_local_mac_del (struct zebra_vrf *zvrf,
                                     zebra_vni_t *zvni, zebra_mac_t *zmac)
{
  zebra_neigh_t               *n = NULL;
  struct listnode             *node = NULL;
      ...
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          if (IS_ZEBRA_NEIGH_ACTIVE (n))
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug ("%u: neigh %s (MAC %s) on VNI %u is now INACTIVE",
                            zvrf_id (zvrf),
                            ipaddr2str (&n->ip, buf2, sizeof(buf2)),
                            prefix_mac2str (&n->emac, buf, sizeof (buf)),
                            zvni->vni);
        ...
        {
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_err ("%u: local MAC %s getting deleted on VNI %u has remote neigh %s",
                       zvrf_id (zvrf),
                       prefix_mac2str (&n->emac, buf, sizeof (buf)),
                       zvni->vni,
                       ipaddr2str (&n->ip, buf2, sizeof(buf2)));
...
static void
zvni_process_neigh_on_remote_mac_add (struct zebra_vrf *zvrf,
                                      zebra_vni_t *zvni, zebra_mac_t *zmac)
{
  zebra_neigh_t               *n = NULL;
  struct listnode             *node = NULL;
      ...
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          if (IS_ZEBRA_NEIGH_ACTIVE (n))
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug ("%u: neigh %s (MAC %s) on VNI %u INACTIVE",
                            zvrf_id (zvrf),
                            ipaddr2str (&n->ip, buf2, sizeof(buf2)),
                            prefix_mac2str (&n->emac, buf, sizeof (buf)),
                            zvni->vni);
...
static void
zvni_process_neigh_on_remote_mac_del (struct zebra_vrf *zvrf,
                                      zebra_vni_t *zvni, zebra_mac_t *zmac)
{
  zebra_neigh_t               *n = NULL;
  struct listnode             *node = NULL;
  ...
  for (ALL_LIST_ELEMENTS_RO (zmac->neigh_list, node, n))
    {
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_err ("%u: remote  MAC %s getting deleted on VNI %u has local neigh %s",
                       zvrf_id (zvrf),
                       prefix_mac2str (&n->emac, buf, sizeof (buf)),
                       zvni->vni,
                       ipaddr2str (&n->ip, buf2, sizeof(buf2)));
...
static int
zvni_del_macip_for_intf (struct interface *ifp,
                         zebra_vni_t *zvni)
{
  struct zebra_vrf          *zvrf = NULL;
      ...
      if (c->address->family == AF_INET)
        {
          ...
          memcpy (&(ip.ipaddr_v4), &(c->address->u.prefix4), sizeof (struct in_addr));
        }
      else if (c->address->family == AF_INET6)
        {
          ip.ipa_type = IPADDR_V6;
          memcpy (&(ip.ipaddr_v6), &(c->address->u.prefix6), sizeof (struct in6_addr));
        }
      else
        {
          continue;
...
static int
zvni_add_macip_for_intf (struct interface *ifp,
                         zebra_vni_t *zvni)
{
  struct zebra_vrf          *zvrf = NULL;
  struct listnode           *cnode = NULL, *cnnode = NULL;
 ...
 */
static int
zvni_gw_macip_add (struct interface *ifp,
                   zebra_vni_t *zvni,
                   struct ethaddr *macaddr,
                   struct ipaddr *ip)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("%u:SVI %s(%u) VNI %u, sending GW MAC %s IP %s add to BGP",
                ifp->vrf_id, ifp->name, ifp->ifindex, zvni->vni,
                prefix_mac2str (macaddr, NULL, ETHER_ADDR_STRLEN),
                ipaddr2str (ip, buf2, sizeof (buf2)));

  zvni_neigh_send_add_to_client (zvrf, zvni->vni, ip, macaddr, ZEBRA_MAC_TYPE_GW);
 ...
 */
static int
zvni_gw_macip_del (struct interface *ifp,
                   zebra_vni_t *zvni,
                   struct ipaddr *ip)
{
  struct zebra_vrf                  *zvrf = NULL;
  ...
  zvni_neigh_send_del_to_client (zvrf, zvni->vni, &n->ip, &n->emac, ZEBRA_MAC_TYPE_GW);
...
static void
zvni_gw_macip_del_for_vni_hash (struct hash_backet *backet, void *zvrf)
{
...
static void
zvni_gw_macip_add_for_vni_hash (struct hash_backet *backet, void *zvrf)
{
  zebra_vni_t                 *zvni = NULL;
  struct zebra_if             *zif = NULL;
  struct zebra_l2info_vxlan   zl2_info ;
      ...
      if (wctx->upd_client && (mac->flags & ZEBRA_MAC_LOCAL))
        {
          ...
          zvni_mac_send_del_to_client (wctx->zvrf, wctx->zvni->vni,
                                       &mac->macaddr, (sticky ? ZEBRA_MAC_TYPE_STICKY : 0));
        }
 ...
 */
void
zebra_vxlan_print_neigh_vni (struct vty *vty, struct zebra_vrf *zvrf, vni_t vni, u_char use_json)
{
  zebra_vni_t *zvni;
  u_int32_t num_neigh;
  struct neigh_walk_ctx wctx;
  ...
  if (use_json)
    {
      vty_out (vty, "%s%s", json_object_to_json_string_ext (json, JSON_C_TO_STRING_PRETTY),
               VTY_NEWLINE);
                                 ...
                                 u_char use_json)
{
  ...
  hash_iterate(zvrf->vni_table,
              (void (*)(struct hash_backet *, void *))zvni_print_neigh_hash_all_vni,
              args);
  if (use_json)
    {
      vty_out (vty, "%s%s", json_object_to_json_string_ext (json, JSON_C_TO_STRING_PRETTY),
...
zebra_vxlan_print_specific_neigh_vni (struct vty *vty, struct zebra_vrf *zvrf,
                                      vni_t vni, struct ipaddr *ip, u_char use_json)
{
  zebra_vni_t *zvni;
  zebra_neigh_t *n;
  json_object *json = NULL;
...
zebra_vxlan_print_neigh_vni_vtep (struct vty *vty, struct zebra_vrf *zvrf,
                                  vni_t vni, struct in_addr vtep_ip, u_char use_json)
{
  zebra_vni_t *zvni;
  u_int32_t num_neigh;
  struct neigh_walk_ctx wctx;
 ...
 */
void
zebra_vxlan_print_macs_vni (struct vty *vty, struct zebra_vrf *zvrf,
...
zebra_vxlan_print_macs_all_vni_vtep (struct vty *vty, struct zebra_vrf *zvrf,
                                     struct in_addr vtep_ip, u_char use_json)
{
  struct mac_walk_ctx             wctx;
  json_object *json = NULL;
 ...
 */
void
zebra_vxlan_print_vnis (struct vty *vty, struct zebra_vrf *zvrf, u_char use_json)
{
  u_int32_t num_vnis;
  json_object *json = NULL;
  void *args[2];
  ...
  if (use_json)
    {
      json = json_object_new_object ();
      json_object_string_add (json, "advertiseGatewayMacip", zvrf->advertise_gw_macip ? "Yes" : "No");
      json_object_int_add (json, "numVnis", num_vnis);
    }
  else
    {
      vty_out(vty, "Advertise gateway mac-ip: %s%s", zvrf->advertise_gw_macip ? "Yes" : "No", VTY_NEWLINE);
  ...
  hash_iterate(zvrf->vni_table, (void (*)(struct hash_backet *, void *))zvni_print_hash, args);
                             ...
                             struct ipaddr *ip)
{
  ...
  if (!zmac)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_err ("%u: trying to del a neigh %s without a mac %s on VNI %u",
                  ifp->vrf_id, ipaddr2str (ip, buf, sizeof (buf)),
  ...
  if (CHECK_FLAG (zmac->flags, ZEBRA_MAC_AUTO) || !listcount (zmac->neigh_list))
                                    ...
                                    u_char ext_learned)
{
  ...
  if (!zmac)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("%u: AUTO MAC %s created for neigh %s on VNI %u",
                    ifp->vrf_id, prefix_mac2str (macaddr, buf, sizeof (buf)),
                    ipaddr2str (ip, buf2, sizeof (buf2)),
                    zvni->vni);
  ...
  /* Before we program this in BGP, we need to check if MAC is locally learnt as well */
  if (!CHECK_FLAG (zmac->flags, ZEBRA_MAC_LOCAL))
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("%u: Skipping neigh %s add to client as MAC %s is not local on VNI %u",
                    ifp->vrf_id, ipaddr2str (ip, buf2, sizeof (buf2)),
  ...
  while (l < length)
    {
      ...
      if (n)
      ...
      else
        {
          if (CHECK_FLAG (mac->flags, ZEBRA_MAC_REMOTE))
            {
              zvni_process_neigh_on_remote_mac_del (zvrf, zvni, mac);
                                 ...
                                 vlanid_t vid)
{
  ...
  /* Remove MAC from BGP. */
  sticky = CHECK_FLAG (mac->flags, ZEBRA_MAC_STICKY) ? 1: 0;
  zvni_mac_send_del_to_client (zvrf, zvni->vni, macaddr, (sticky ? ZEBRA_MAC_TYPE_STICKY : 0));
  ...
  if (mac)
    {
      if (CHECK_FLAG (mac->flags, ZEBRA_MAC_LOCAL))
        ...
        {
          /*
           * If we have already learned the MAC as a remote sticky MAC,
 ...
 */
int
zebra_vxlan_add_del_gw_macip (struct interface *ifp,
                              struct prefix *p,
                              int add)
{
  struct ipaddr               ip ;
  ...
  if (IS_ZEBRA_IF_MACVLAN (ifp))
    {
      ...
      struct zebra_if             *ifp_zif = NULL; /* Zebra daemon specific info for MACVLAN */
...
/*
 * Handle message from client to enable/disable advertisement of g/w macip routes
 */
  ...
  if (!vni)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("%u:EVPN gateway macip Adv %s, currently %s",
                    ...
                    advertise_gw_macip_enabled(zvrf, NULL) ? "enabled" : "disabled");
      ...
      if (advertise_gw_macip_enabled (zvrf, zvni))
        ...
        hash_iterate (zvrf->vni_table, zvni_gw_macip_del_for_vni_hash, zvrf);
  ...
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("%u:EVPN gateway macip Adv %s on VNI %d , currently %s",
                    zvrf_id (zvrf), advertise ? "enabled" : "disabled", vni,
                    advertise_gw_macip_enabled(zvrf, zvni) ? "enabled" : "disabled");
